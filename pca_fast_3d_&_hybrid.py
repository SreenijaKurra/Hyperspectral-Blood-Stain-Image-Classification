...
Original file is located at
    https://colab.research.google.com/drive/1eQF0BdvS1XNlo6RCxmRGR_rjcBC-tHgf
"""
...
'''def DLMethod(method, HSI, NC = 75):
    RHSI = np.reshape(HSI, (-1, HSI.shape[2]))
    if method == 'FA': ## Factor Analysis
        fa = FactorAnalysis(n_components = NC, random_state = 2019)
        RHSI = fa.fit_transform(RHSI)
        RHSI = np.reshape(RHSI, (HSI.shape[0], HSI.shape[1], NC))
    return RHSI'''
...
## Writing Confusion Matrix
start_timehym = time.time() ##Timer Count starts
file_name = str(WS)+str(DLM)+str(k)+"_Confusion_Matrix_with_biggest_font.png"
cm, annot = Conf_Mat(Te_Pred, TeC, target_names)
cm.index.name = 'Actual'
cm.columns.name = 'Predicted for Hybrid CNN'
fig, ax = plt.subplots(figsize=(14,14))
sns.set(font_scale=1.2)
sns.heatmap(cm, cmap= "Spectral", annot=annot, fmt='', ax=ax, linewidths=0.6, annot_kws={"size": 18})
plt.savefig(file_name, dpi=500)
#files.download(file_name)
end_timehym = time.time()##Timer Ends
run_timehym = end_timehym - start_timehym
print("The code took", run_timehym, "seconds to run.")
...
print("The Reduce the Dimensionality           :", DL_Time, "seconds to run.\n")
print("*******************************************************************************\n")
#A Fast and Compact 3D CNN Model
print("A Fast and Compact 3D CNN Model         :", run_timeF3D, "seconds to run.")
print("Fast 3D - The Train & Fit               :", Tr_Time_F3D, "seconds to run.")
print("Fast 3D - The Prediction                :", Te_Time_Te, "seconds to run.")
print("Fast 3D - Confusion Matrix              :", run_timef3m, "seconds to run.\n")
print("A Fast and Compact 3D CNN Model Accuracy:")
print(classification_f3d)
print("*******************************************************************************\n")
#Hybrid CNN (3D followed by 2D CNN) Model
print("Hybrid CNN Model                        :", run_timeHy, "seconds to run.")
print("Hybrid - The Train & Fit                :", Tr_Time_Hy, "seconds to run.")
print("Hybrid - The Prediction                 :", Te_Time_Hy, "seconds to run.")
print("Hybrid - Confusion Matrix               :", run_timehym, "seconds to run.\n")
print("Hybrid CNN Model Accuracy:")
print(classification_hy)
print("*******************************************************************************\n")
print("The Whole code                          :", run_time1, "seconds to run.")
print("The Whole code                          :", run_time1/60, "minutes to run.")